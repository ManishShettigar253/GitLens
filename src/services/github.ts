...
export async function fetchUserPullRequests(
  ...
  // 4. Fetch User's Repositories to capture direct-commit repos
  try {
    ...
    if (reposRes.ok) {
      ...
      userRepos.forEach((repo: any) => {
        if (!existingRepos.has(repo.name.toLowerCase())) {
          // Generate baseline contribution entry for direct-commit repository
          ...
          mappedPRs.push({
            id: repoId,
            number: 1,
            title: `Direct commits to ${repo.name}`,
            state: 'merged',
          ...
          if (pushedDate.getTime() - createdDate.getTime() > 1000 * 60 * 60 * 24 * 7) {
            ...
            mappedPRs.push({
              id: updateId,
              number: 2,
              title: `Ongoing commits in ${repo.name}`,
              state: 'merged',
      ...
      // Re-sort after adding direct commits
      mappedPRs.sort((a, b) => new Date(b.created_at).getTime() - new Date(a.created_at).getTime());
    }
  } catch (e) {
